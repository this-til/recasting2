...
public class JadeDomainSlashArts extends ExtendedSlashArts {
    ...
    private void stopDomain(LivingEntity caster) {
        ITimeRun timeRun = RecastingAttachments.timeRun(caster);
        ...
        RecastingAttachments.buffStackData(caster)
                .setLevel(RecastingBuffTypes.JADE_DOMAIN.get(), 0, caster.level());
    }
    ...
    private void applyJadeFire(LivingEntity target, LivingEntity caster) {
        IBuffStackData buffData = RecastingAttachments.buffStackData(target);
    ...
    private class DomainTick implements Runnable {
        ...
        public void run() {
            ...
            boolean hasDomainBuff = RecastingAttachments.buffStackData(caster)
                    .getLevel(RecastingBuffTypes.JADE_DOMAIN.get(), caster.level()) > 0;
            if (!hasDomainBuff) {
                stopDomain(caster);
                return;
            }
            ...
            if (BladeStateAccess.of(mainHand).isEmpty()) {
                return;
            }

            triggerBladeReleaseAtRandomTarget(
                    caster,
                    BladeStateAccess.of(mainHand)
