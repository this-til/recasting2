...
public class JudgementCut {
    static public EntityJudgementCut doJudgementCutJust(LivingEntity user, float modifiedRatio, float extraDamage) {
        EntityJudgementCut sa = doJudgementCut(user, modifiedRatio, extraDamage);
        sa.setIsCritical(true);
        return sa;
    }

    static public EntityJudgementCut doJudgementCut(LivingEntity user, float modifiedRatio, float extraDamage) {
        ...
        Optional<Vec3> resultPos = stack.getCapability(ItemSlashBlade.BLADESTATE).filter(s -> s.getTargetEntity(worldIn) != null).map(s -> Objects.requireNonNull(s.getTargetEntity(worldIn)).getEyePosition(1.0f));
        ...
        user.getCapability(ConcentrationRankCapabilityProvider.RANK_POINT).ifPresent(rank -> jc.setRank(rank.getRankLevel(worldIn.getGameTime())));

        //noinspection ConstantValue
        if (jc instanceof IEntityModifiedRatio entityModifiedRatio) {
            entityModifiedRatio.setRecasting$modifiedRatio(modifiedRatio);
        }
        jc.setDamage(extraDamage);
