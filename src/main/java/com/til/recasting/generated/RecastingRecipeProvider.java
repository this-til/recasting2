...
/**
 * Recasting 模组的配方生成器
 * 从 RecastingRecipes 常量类中收集 RecipeBuilderWrapper 并生成配方
 * 配方ID自动使用字段名转小写
 */
@Log4j2
public class RecastingRecipeProvider extends RecipeProvider implements IConditionBuilder {
    ...
    protected void buildRecipes(@NotNull Consumer<FinishedRecipe> consumer) {
        ...
        for (RecipeBuilderEntry entry : recipeBuilders) {
            try {
                ...
                log.debug("已生成配方: {} (字段名: {})", recipeId, fieldName);
            } catch (Exception e) {
                log.error("生成配方时发生错误，字段名: {}", entry.fieldName, e);
            }
        }
    ...
    /**
     * 使用反射获取 RecastingRecipes 中所有的 RecipeBuilderWrapper 静态字段
     */
    private List<RecipeBuilderEntry> getRecipeBuilderWrappers() {
        List<RecipeBuilderEntry> entries = new ArrayList<>();
        Field[] allFields = RecastingRecipes.class.getDeclaredFields();
