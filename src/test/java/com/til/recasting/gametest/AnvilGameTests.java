...
/**
 * 铁砧 SE 铭刻、特殊 SE 提取、SA 提取的正/负向用例。
 */
...
public final class AnvilGameTests {
    ...
    @GameTest(template = "empty", batch = "recastingAnvil")
    public static void extractSpecialSe_withAbyssFlame(GameTestHelper helper) {
        ...
        try {
            AnvilTestHelper.assertHasOutput(output, "extractSpecialSe_withAbyssFlame");
    ...
    @GameTest(template = "empty", batch = "recastingAnvil")
    public static void extractSpecialSe_withoutSpecial_noOutput(GameTestHelper helper) {
        ItemStack blade = TestItemFactory.bladeFromDefinition(
                helper.getLevel().registryAccess(),
                SlashBladeDefinitions.BROADSWORD_WOOD.getName()
        );
        ...
        ItemStack output = AnvilTestHelper.preview(blade, abyss, helper.makeMockPlayer());
        try {
            AnvilTestHelper.assertNoOutput(output, "extractSpecialSe_withoutSpecial_noOutput");
        } catch (AssertionError error) {
            helper.fail(error.getMessage());
            return;
        }
