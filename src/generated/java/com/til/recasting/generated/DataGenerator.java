...
public class DataGenerator {
    ...
    public static void onGatherData(GatherDataEvent event) {
        ...
        final RegistrySetBuilder bladeBuilder = new RegistrySetBuilder()
                .add(SlashBladeDefinition.REGISTRY_KEY, SlashBladeDefinitions::registerAll);
        ...
        dataGenerator.addProvider(
                event.includeServer(),
                namedProvider("Recasting Recipes", new RecastingRecipes(packOutput, lookupProvider))
        );

        dataGenerator.addProvider(
                event.includeServer(),
                namedProvider("Recasting SlashBlade Recipes", new SlashBladeRecipes(packOutput, lookupProvider))
        );

        dataGenerator.addProvider(
                event.includeServer(),
                namedProvider("Recasting Special Effect Recipes", new SpecialEffectRecipes(packOutput, lookupProvider))
        );
