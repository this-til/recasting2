...
本文档记录了 Recasting2（1.21 NeoForge）项目的开发规则、约定和重要注意事项。
...
- `SlashBlade_Resharped/` —— SlashBlade: Resharped 源码参考目录（`https://github.com/0999312/SlashBlade_Resharped`，分支 `1.21.1`），适用上述只读规则；已写入 `.gitignore`，不纳入本仓库版本管理
- **构建产物与 IDE 缓存不纳入版本管理** —— `build/`、`.gradle/`、`.idea/`、`bin/`、`out/`、`run/`、`run-server/`、`run-data/` 等



### 1.21 移植现状

- **目标环境** —— Minecraft **1.21.1** + NeoForge（ModDevGradle）+ Java **21**；强制依赖 SlashBlade Resharped **2.0+**
- **姊妹工程** —— 玩法完整实现仍以 `D:\Project\MC\Recasting2`（Forge 1.20.1 / 分支 `1.20`）为移植源；本仓库分支 `1.21` 为 NeoForge 移植目标

## Java 编码规范

- **项目 Java 版本** —— Java 21；可使用 `record`、`instanceof` 模式匹配、`switch` 表达式、`var`（类型显而易见时）、以及 Java 21 合法语法
...
- **浮点数相等比较使用 epsilon** —— 禁止直接用 `==` / `!=` 比较 `float` / `double` 是否相等；`>`、`<`、`>=`、`<=` 可正常使用。优先项目内既有工具方法；若无则对照 SlashBlade / 原版等价写法，勿引入无依据的第三方 helper
...
- **可空语义用** `@Nullable` **标注** —— 参数或返回值可能为 `null` 时，用注解标明；调用方与实现保持一致（包名以项目现用为准，移植时对齐 NeoForge / JetBrains 注解习惯）
...
- **注册项在使用点直接** `get()` —— 不推荐 `T tmp = XxxRegistry.FOO.get();` 再使用 `tmp`；应在调用处直接写 `XxxRegistry.FOO.get()`。适用于 `DeferredHolder` / `Supplier` 等注册项（如 `RecastingBuffTypes.`*、`RecastingEntities.*`、`RecastingItems.*`、`RecastingAttackTypes.*`、`SlashArtsRegistry.*`）。对整个注册表 `REGISTRY.get()` 后多次查找，可局部持有注册表引用，不在此列



## Minecraft / NeoForge 约定
...
- **逻辑侧必须区分客户端与服务端** —— 修改世界、实体、附加数据、网络同步等须在服务端执行；用 `!level().isClientSide()` / `level().isClientSide()`，或 NeoForge 的 `@EventBusSubscriber` / `Dist` 区分。客户端专用渲染、粒子、按键等放在 `client/` 包下
- **模组元数据** —— 使用 `src/main/templates/META-INF/neoforge.mods.toml`（由 `generateModMetadata` 展开），不要手写 `mods.toml` 当主入口
- **本地化条目写在** `LanguageItems` —— 玩家可见文案在 `LanguageItems` 中维护翻译，不直接手改生成的语言文件（内容移植到位后恢复此约定）
...
- **扩展数据** —— 优先对齐 SlashBlade 2.0 / NeoForge 的附加数据与序列化方式（移植时对照 `SlashBlade_Resharped/`）；不往未约定字段硬塞未序列化数据
- **延迟序列** —— `TIME_RUN` 等定时能力的 `addTimerCell(...)`；实体 buff 用 `RecastingBuffTypes` + `BUFF_STACK_DATA`（内容移植时按 1.21 API 落地）
- **Mixin 边界** —— 仅用于定义扩展、攻击路由、CODEC 序列化、tooltip、JEI 兼容；目标类在 `mods.flammpfeil.slashblade.`* 时 `remap = false`；新 Mixin 必须登记 `recasting.mixins.json`
